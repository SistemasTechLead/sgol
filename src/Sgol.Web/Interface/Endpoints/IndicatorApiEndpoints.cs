...
public static class IndicatorApiEndpoints
{
    private static readonly HashSet<string> AllowedParameters = new(
        ["isoYear", "isoWeek", "level", "responsiblePersonId", "cursor", "limit"],
        StringComparer.Ordinal);

    public static IEndpointRouteBuilder MapIndicatorApi(this IEndpointRouteBuilder endpoints)
    {
        endpoints.MapGet("/api/v1/indicators", ReadAsync);
        return endpoints;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!TryRequest(context.Request.Query, actor, out var request))
            return Problem(context, 400, "FILTRO_INDICADORES_INVALIDO", "Los filtros de indicadores no son válidos");
        ...
        catch (IndicatorAccessDeniedException)
        {
            return Problem(context, 403, "ACCESO_DENEGADO", $"Se requiere {IndicatorAuthorization.View}");
        }
        catch (IndicatorQueryInconsistentException)
        {
            return Problem(context, 500, "CONSULTA_INDICADORES_INCONSISTENTE", "No fue posible consultar los indicadores");
        }
    }
