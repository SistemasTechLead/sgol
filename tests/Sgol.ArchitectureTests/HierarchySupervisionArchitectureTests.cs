...
public sealed class HierarchySupervisionArchitectureTests
{
    [Fact]
    public void SupervisionContractsRemainHostAndProviderIndependent()
    {
        ...
        var source = File.ReadAllText(Path.Combine(module, "Contracts", "HierarchySupervision.cs"));
    ...
    [Fact]
    public void Hu031AddsOnlyApprovedReadSurface()
    {
        var root = ArchitectureBoundaryTests.FindRepositoryRoot(AppContext.BaseDirectory);
        var web = Path.Combine(root, "src", "Sgol.Web");
        var endpoint = File.ReadAllText(Path.Combine(web, "Interface", "Endpoints", "HierarchySupervisionApiEndpoints.cs"));
        var reader = File.ReadAllText(Path.Combine(web, "Infrastructure", "Persistence", "Execution", "EfObligationQueryReader.cs"));

        Assert.Equal(2, Count(endpoint, "MapGet("));
        Assert.DoesNotContain("MapPost(", endpoint, StringComparison.Ordinal);
        Assert.Contains("/api/v1/supervision/obligations", endpoint, StringComparison.Ordinal);
        Assert.Contains("/api/v1/validations/pending", endpoint, StringComparison.Ordinal);
        Assert.DoesNotContain("/api/v1/indicators", endpoint, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain("InternalNotice", endpoint, StringComparison.Ordinal);
        Assert.DoesNotContain("Outbox", endpoint, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain(migrations, path => Path.GetFileName(path).Contains("Supervision", StringComparison.OrdinalIgnoreCase));
    }
