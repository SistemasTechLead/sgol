...
internal sealed partial class Cv05Infrastructure(string imageId) : IAsyncDisposable
{
    ...
    private readonly HashSet<string> unhealthyChecks = new(StringComparer.Ordinal);
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await CreateNetworkAsync(cancellationToken);

            phase = "POSTGRESQL";
            primaryUsername = $"cv05_{Guid.CreateVersion7():N}";
            primaryPassword = Convert.ToHexString(RandomNumberGenerator.GetBytes(24));
            postgres = new PostgreSqlBuilder(DemoContract.PostgreSqlImage)
                .WithDatabase(databaseName)
                .WithUsername(primaryUsername)
                .WithPassword(primaryPassword)
                .WithNetwork(network)
                .WithNetworkAliases("cv05-postgres")
                .Build();
            try
            {
            ...
            {
                throw new DemoFailureException("POSTGRESQL", "NONE", "CV05_POSTGRESQL_FAILED");
            }
            ...
            await using (var context = CreateContext())
            {
                ...
                {
                    throw new DemoFailureException("POSTGRESQL", "NONE", "CV05_DATABASE_CONTRACT_FAILED");
                }
            }

            await StartRestorePostgreSqlAsync(cancellationToken);
            phase = "STORES";
            await StartStoresAsync(cancellationToken);
            phase = "ANTIMALWARE";
            await StartScannerAsync(cancellationToken);
...
#pragma warning restore CA1309
        ...
        catch (Exception exception)
        {
            ...
            {
                "POSTGRESQL" => "CV05_DATABASE_CONTRACT_FAILED",
                "STORES" or "ANTIMALWARE" => "CV05_STORE_FAILED",
                "HTTPS" => "CV05_HTTPS_FAILED",
                _ => "CV05_PRECONDITION_FAILED",
            };
            var failure = new DemoFailureException(phase, "NONE", code);
            failure.Data["Category"] = exception.GetType().Name;
            failure.Data["Health"] = HealthIssues();
            throw failure;
        }
    }
    ...
    public async Task<bool> CleanupAsync()
    {
        ...
        success &= await CleanupStoresAsync();
    ...
    private async Task RunDirectionBootstrapAsync(CancellationToken cancellationToken)
    {
        ...
        startInfo.Environment["SGOL_BOOTSTRAP_PERSON_CODE"] = "DIR-CV05";
        startInfo.Environment["SGOL_BOOTSTRAP_PERSON_DISPLAY_NAME"] = "Dirección sintética CV-05";
        ...
        if (exit != 0)
        {
            throw new DemoFailureException("SEED", "NONE", "CV05_DATABASE_CONTRACT_FAILED", exit);
        }
    }
    ...
    private async Task StartWebAsync(CancellationToken cancellationToken)
    {
        ...
        certificatePath = Path.Combine(Path.GetTempPath(), $"sgol-cv05-{Guid.CreateVersion7():N}.pfx");
        var certificateBytes = certificate.Export(X509ContentType.Pfx, certificatePassword);
        certificatePfxBase64 = Convert.ToBase64String(certificateBytes);
        ...
        startInfo.Environment["ASPNETCORE_ENVIRONMENT"] = "CI";
        startInfo.Environment["ASPNETCORE_URLS"] = BaseAddress.AbsoluteUri;
        startInfo.Environment["ConnectionStrings__Sgol"] = ConnectionString;
        AddEvidenceEnvironment(startInfo);
        ...
        if (!web.Start())
        {
            throw new DemoFailureException("HTTPS", "NONE", "CV05_HTTPS_FAILED");
        }
        stdoutDrain = DrainWebAsync(web.StandardOutput);
        stderrDrain = DrainWebAsync(web.StandardError);
    ...
    private async Task WaitUntilReadyAsync(CancellationToken cancellationToken)
    {
        ...
        if (live)
        {
            var failure = new DemoFailureException("STORES", "NONE", "CV05_STORE_FAILED");
            failure.Data["Health"] = HealthIssues();
            throw failure;
        }
        throw new DemoFailureException("HTTPS", "NONE", "CV05_HTTPS_FAILED");
    ...
    private static string FindRepositoryRoot()
    {
        ...
        return directory?.FullName ?? throw new DemoFailureException("PREFLIGHT", "NONE", "CV05_PRECONDITION_FAILED");
    }

    private async Task DrainWebAsync(StreamReader reader)
    {
        while (await reader.ReadLineAsync() is { } line)
        {
            if (!line.Contains("Unhealthy", StringComparison.OrdinalIgnoreCase)) continue;
            foreach (var name in new[] { "evidence-storage", "data-protection", "postgresql" })
                if (line.Contains(name, StringComparison.OrdinalIgnoreCase))
                    lock (unhealthyChecks) unhealthyChecks.Add(name);
        }
    }

    private string HealthIssues()
    {
        lock (unhealthyChecks) return string.Join(',', unhealthyChecks.Order(StringComparer.Ordinal));
    }
