...
El primer run remoto del PR `#59` (`35795581552`) no llegó a CV-05 ni a los uploads: el gate HU-035 encontró PostgreSQL apagándose tras aceptar `pg_isready` por socket durante la inicialización. Con autorización expresa se cambió sólo el healthcheck del aprovisionador sintético a TCP `127.0.0.1`, que exige el servidor final; la regresión enfocada falló antes y pasó después. Los límites, intervalos y reintentos siguen iguales. Ese run no aporta evidencia sobre cuota de artifacts.
