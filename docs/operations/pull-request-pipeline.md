...
### Excepción explícita y puntual de cuota para PR #58 (`TECH-E2E-CV-04`)

El responsable autorizó el 2026-09-22 una salida sin cargos ni espera del bloqueo de cuota de GitHub Actions. Sólo en PR `#58`, los dos pasos `actions/upload-artifact` se sustituyen por publicaciones verificadas en el resumen y el log del mismo run. Los demás PR conservan sus pasos y contratos habituales. El cambio no omite demo, formato, escaneo, OCI, TECH-OPS, HU-035 ni cleanup; no convierte un fallo en éxito.

El primer publicador exige el JSON y Markdown sanitizados, SHA exacto, dos ciclos, escenarios completos, huella idéntica y cleanup `PASSED`; entonces escribe la matriz cerrada, el estado y los SHA-256 de ambos reportes en resumen y log. El segundo exige la existencia no vacía del layout OCI, metadata y evidencias públicas operativas, y publica únicamente sus nombres cerrados, tamaños y SHA-256. Cualquier ausencia o contrato inválido falla el check. Los resúmenes y logs de GitHub Actions no consumen la cuota de artifacts; su retención depende de la configuración del repositorio.

Esta excepción **no equivale** a conservar el layout OCI descargable ni los archivos JSON/Markdown originales durante 14 días. El SHA-256 demuestra lo que produjo el runner, pero no permite recuperar el contenido binario después del job. Esa diferencia se informa al revisar el PR; no se atribuye falsamente cumplimiento de los apartados de retención de la Adenda 42 ni del gate operativo histórico. La decisión del responsable sobre aceptar esta excepción para el cierre sigue siendo explícita y separada del merge.
