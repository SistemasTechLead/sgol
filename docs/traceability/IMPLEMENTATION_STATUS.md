...
| Punto de avance | `FRONT-002` integrada mediante PR `#68`; `FRONT-003` implementada localmente en `codex/front-003`, con publicación autorizada e integración pendiente de pipeline verde para la cabeza exacta y segunda autorización de merge. `FRONT-004`, `TECH-FRONT-005` y las historias posteriores no se inician |
...
| `FRONT-003` | `F07_ADENDA_45_BACKLOG_FRONTEND_DEFINITIVO.md`; `HU-001`, `CA-001`, `CP-001-P/N`; Adenda 46 y extensión mínima de `docs/design` aprobada por el responsable en este chat | `FRONT-002` integrada; `GET/POST /api/v1/people` y `GET /api/v1/people/{id}` existentes | `Implementada localmente` en `codex/front-003`: UI-I01 lista, alta y detalle con historial; navegación visible sólo para Dirección con `PER-PERSONA-ADMIN` proyectado; el backend conserva la autoridad, CSRF, idempotencia y auditoría. Lista API incorpora `meta.count` exigido por cliente común. Build Release `26/26`, unitarias enfocadas, arquitectura, PostgreSQL `13/13` y categoría completa `FRONT_BROWSER` con cinco pruebas; formato dirigido y diff en `docs/traceability/FRONT_003_PERSONAS.md`. No implementa edición/baja/reactivación ni UI-I02..I07. Integración pendiente de pipeline verde para la cabeza exacta y segunda autorización de merge |
| `FRONT-004..020` | `F07_ADENDA_45_BACKLOG_FRONTEND_DEFINITIVO.md` | Según orden y dependencias de la Adenda 45 | `No iniciadas`; cada brecha bloquea sólo su historia consumidora |
