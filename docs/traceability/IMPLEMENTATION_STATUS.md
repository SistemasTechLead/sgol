...
| Punto de parada | `TECH-FRONT-004` implementa sólo el harness y su smoke del shell/401. Commit, push y PR fueron autorizados separadamente; el merge requiere una segunda autorización después de pipeline verde para el SHA exacto. No se inicia `FRONT-001` ni `TECH-FRONT-005` |
...
| `TECH-FRONT-004` | `F07_ADENDA_45_BACKLOG_FRONTEND_DEFINITIVO.md` | Historias con navegador | `Implementada localmente` en `codex/tech-front-004`: harness Playwright de Kestrel HTTPS y PostgreSQL desechable, cuatro fixtures de sesión hospedada, Chromium escritorio/WebKit móvil, shell vacío, 401, teclado/foco, landmarks, comprobación de accesibilidad crítica, capturas enmascaradas y cleanup. Build Release de 13 proyectos sin errores ni advertencias, smoke enfocado `1/1` con ocho combinaciones y arquitectura afectada `9/9` verdes; comandos y mapa en `TECH_FRONT_004_HARNESS.md`. El commit que contiene esta actualización y su PR acreditan publicación; pipeline exacto e integración requieren evidencia posterior. Suite funcional completa y login visual quedan fuera de alcance |
