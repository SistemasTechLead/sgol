...
| `FRONT-011` | Fila 79 de Adenda 45; HU-012/017, CA/CP-012/017, F06, Adendas 44/46 y extensión consumidora BR-API09/UI-C06/C07 aprobada para esta tarea | `FRONT-010` integrada mediante PR `#76`; rama aislada `codex/front-011` desde `origin/master` `2443e121c46238433adffd63bc905f8bfccfee2e` | `Implementada localmente`: lectura autorizada con ETag, versión vigente e historia de activación y elegibilidad por las ocho TAR; editores cerrados con CSRF, `Idempotency-Key`, `If-Match`, replay y 412 con recarga y decisión explícita en `/configuracion`. La proyección de sesión incluye los dos permisos de Dirección; el servidor conserva autorización, historia, auditoría, concurrencia y no efecto. Build Release 0 errores/advertencias; unitarias 23/23, PostgreSQL 9/9, arquitectura 4/4 y `FRONT_BROWSER` completo 15/15 con Kestrel HTTPS y PostgreSQL real; formato dirigido y `git diff --check` se registran en `FRONT_011_POLITICAS_TAR.md`. Cambios sin commit, push, PR ni merge; publicación y CI remotos diferidos hasta autorización. |
