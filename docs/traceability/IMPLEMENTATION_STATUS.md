...
| Bloqueo diagnóstico vigente | La corrección del wrapper de `age` debe superar el gate integral Linux AMD64 sobre el SHA publicado. El run causal `35400323775` mantuvo verdes protección de `Fuentes/`, Gitleaks, build, pruebas, formato, vulnerabilidades, TECH-OPS y la sonda `1/1`, pero no acredita cierre porque HU-035 se detuvo durante la exportación del caso positivo |
| Validación local de las correcciones | La corrección final aprobó sintaxis; red y wrapper `80` aserciones; diagnóstico `216`; stream puro `7/7`; contrato HU-035 `5/5`; sonda real de réplica `1/1`; reproducción/regresión del lector funcional y atribución de `EXPORT_BACKUP_IO_ERROR` con PostgreSQL real `1/1`; sonda Docker de stdin sin/con `--interactive`; build Release `23/23`, cero errores/advertencias; Unit `26/26`; Architecture `5/5`; validadores HU-035 y TECH-OPS; formato, protección local de `Fuentes/` y `git diff --check`. Gitleaks y espejo byte-a-byte permanecen a cargo del runner Linux: Gitleaks no está instalado localmente y `core.autocrlf=true` materializa las copias raíz en CRLF mientras `Fuentes/**` permanece binario/LF; no hay cambios Git dentro de `Fuentes/` |
