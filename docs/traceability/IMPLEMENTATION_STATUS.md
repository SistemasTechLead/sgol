...
| Punto de avance | `FRONT-002` integrada mediante PR `#68`; `FRONT-003` integrada mediante PR `#69`, cabeza `65f3b6bc2a4652f7eb92d167e88b9d079444d702`, run `36041719411` con `TECH-BASE-003 / PR gates` `SUCCESS` para ese SHA y merge `9dba2ed71841c7df443ea773bd986208928d98d1`; cabeza y merge verificados como ancestros de `origin/master` el 2026-09-24. `FRONT-004` integrada mediante PR `#70`, cabeza `aa23c419df907f032b999eeeef7442b99d291dda`, run `36049553112` `SUCCESS` para ese SHA y merge `b291d3ab5dafe651455a5230d2aa7780b875c359`; cabeza y merge ancestros de `origin/master` verificado el 2026-09-24. `FRONT-005` integrada mediante PR `#71`, cabeza `b8e537ff9e7692ac1ead8d0f1028c5fc7e8f6213`, check requerido `SUCCESS` run `36059652548` y merge `6bb9450314bf2c9cf5119d507ef39800b50ef22b`; ambos ancestros de `origin/master` verificado. `FRONT-006` implementada localmente sin commit, push, PR ni merge; `TECH-FRONT-005` y tareas posteriores no iniciadas |
...
| `FRONT-006` | Fila 69 de Adenda 45; `HU-006`, `CA-006`, `CP-006-P/N`; Adendas 46 y 47; extensión mínima de `docs/design/componentes.md` y `estados-y-mensajes.md` aprobada expresamente para UI-I04/I05 | `FRONT-005` integrada mediante PR `#71`; API real `/api/v1/users`, cliente común y sesión request-scoped existentes | `Implementada localmente` en rama aislada `codex/front-006`, sin commit, push, PR ni merge. Dirección lista, crea, desactiva y reactiva cuentas individuales; se usa `PER-USUARIO-ADMIN`, persona vinculada, CSRF e `Idempotency-Key` por intención. `GET /users` añade sólo `meta.count`; servidor genera una contraseña temporal que se muestra una vez y Dirección entrega presencialmente conforme a Adenda 47. Build Release sin errores/advertencias; unitarias de cuentas `8/8`, arquitectura afectada `9/9`, PostgreSQL `10/10` y navegador completo `FRONT_BROWSER` `8/8`. Comandos, mapa, capturas sintéticas sanitizadas y límite del vacío autenticado en `FRONT_006_CUENTAS.md`. Suite integral, pipeline remoto y publicación no ejecutados por alcance local |
| `FRONT-007..020` | `F07_ADENDA_45_BACKLOG_FRONTEND_DEFINITIVO.md` | Según orden y dependencias de la Adenda 45 | `No iniciadas`; cada brecha bloquea sólo su historia consumidora |
