...
| Diagnóstico causal de PUT y despacho REFERENCE | Adenda 40 y complementos aprobados el 2026-09-18: amplían el artifact existente con categorías cerradas del PUT y clasificaciones sanitizadas de red, dirección anunciada, nodo y capacidad; registran resultado cerrado de outbox/job/reconciliación; y clasifican las excepciones no contractuales de captura exclusivamente por siete etapas y seis clases cerradas, sin conservar mensaje ni excepción interna. No cambian S3, reintentos, timeouts ni matriz y no acreditan cierre de HU-035 |
...
| Bloqueo diagnóstico vigente | El run `35395241760` sobre `98e26d17bcb32f1a7a237997d9cdf8eb72d92781` mantuvo verdes TECH-OPS y la sonda de réplica `1/1`; después falló en `REFERENCE` con job `SUCCEEDED` y reconciliación `FAILED / UNEXPECTED_RECONCILIATION_FAILURE`. La causa queda acotada a una excepción no contractual dentro de `CaptureReferenceAsync`; la clasificación aprobada debe identificar etapa y clase en el siguiente SHA exacto. No acredita cierre |
| Validación local de las correcciones | La clasificación causal aprobó sintaxis; red `77` aserciones; diagnóstico `216`; stream puro `7/7`; contrato HU-035 `5/5`; sonda real de réplica `1/1`; build Release `23/23`, cero errores/advertencias; Unit `26/26`; Architecture `5/5`; validadores HU-035 y TECH-OPS; formato, protección local de `Fuentes/` y `git diff --check`. Gitleaks y espejo byte-a-byte permanecen a cargo del runner Linux: Gitleaks no está instalado localmente y `core.autocrlf=true` materializa las copias raíz en CRLF mientras `Fuentes/**` permanece binario/LF; no hay cambios Git dentro de `Fuentes/` |
