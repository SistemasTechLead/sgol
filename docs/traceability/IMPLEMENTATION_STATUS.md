...
| Diagnóstico causal de PUT y despacho REFERENCE | Adenda 40 y complementos aprobados el 2026-09-18: amplían el artifact existente con categorías cerradas del PUT y clasificaciones sanitizadas de red, dirección anunciada, nodo y capacidad después del fallo y antes de limpieza; el complemento registra caso, resultado/error del outbox y estado/error del job con valores cerrados cuando REFERENCE falla. El run `35383233043` sobre `e3a31ebdc2527ef4a720dd9447828f3c00b5b075` mostró red final, dirección final, nodo presente y capacidad positiva, pero falló en REFERENCE con `RetryScheduled`; no cambia S3, outbox, job, reintentos, timeouts ni matriz y no acredita cierre de HU-035 |
| Causas raíz y correcciones | El run `35386054702` sobre `e4f64b7abe2174f36d7836aabd7481339d42652a` demostró que el caso positivo devolvía `RETRY_SCHEDULED / RECOVERY_REFERENCE_JOB_FAILED` sin fila de job: un UUID plano violaba el checkpoint JSON objeto. Corregido el checkpoint, el run `35388790792` sobre `ad8f13abcc31449a2e025b4386ad546961ef9e9a` alcanzó de nuevo el PUT y demostró `FINAL_ONLY / OUTSIDE_FINAL_NETWORK / PRESENT / POSITIVE`: el reinicio con la red final y `bridge` temporal hacía que SeaweedFS anunciara una dirección retirada después. Las correcciones mínimas validan el checkpoint cerrado y fijan como identidad anunciada el alias ya existente de la red final, sin alterar reintentos, estados, operaciones S3, topología ni alcance productivo |
| Bloqueo diagnóstico vigente | El run `35391823674` sobre `c86d62b170c68689d18d7755bb72af9433b567c6` confirmó `FINAL_ONLY / FINAL_NETWORK / PRESENT / POSITIVE`, y todos los pasos previos incluido TECH-OPS y la sonda de réplica `1/1` aprobaron. El gate HU-035 falló después en `REFERENCE`: el evento quedó `FAILED`, pero el artifact no publicó su código cerrado. La ampliación aprobada añade sólo estado/error cerrado del job y de la reconciliación cuando la captura no alcanza `REFERENCE_CAPTURING`; no cambia producción ni acredita cierre |
| Validación local de las correcciones | Para el checkpoint: compilación enfocada sin errores ni advertencias; regresiones puras `2/2`; job operativo `1/1`; arquitectura de continuidad `5/5`; validador HU-035 y diagnóstico de servidor `215` aserciones; tras un único restore locked, build de los 22 proyectos sin errores ni advertencias; formato y protección local de `Fuentes/` aprobados. Para el alias estable: pruebas puras de red y diagnóstico, build exacto, pruebas afectadas, validadores y gate Linux AMD64 del SHA publicado quedan requeridos antes del cierre |
