...
| Bloqueo diagnóstico vigente | El run `35454375124` demostró el falso `RECONCILIATION_IMMUTABLE_CONFLICT` por pérdida de precisión sub-microsegundo; la corrección normaliza la frontera PostgreSQL, conserva el hash y mantiene el rechazo real. El run `35456084194`, SHA `32eb4e15d99ebbbf9e0b7146a68422bd47c9b34e`, aprobó el replay positivo y demostró en `identity_missing` que el rol sintético aislado requería el `SET session_replication_role` ya usado por la matriz; se concedió sólo ese parámetro, sin `SUPERUSER`. El run `35457536271`, SHA `bb35f8bdd78d88b30679a22d2de5e96f04885e5b`, falló el segundo restore; el run `35460899582`, SHA `72695b77a27f2c867c674bbb557ab66fb22168eb`, llegó al restore de `primary_target` y volvió a producir `OPERATIONS_COMMAND_FAILED`. Como cada caso usa el mismo backup inmutable y el override ocurre después del restore, la posición variable demostró una frontera dependiente de timing. `BackupProcessPipeline` permitía que una clausura exitosa del pipe `age -> pg_restore` escapara como `IOException` antes de evaluar exits. La corrección espera ambos procesos y acepta la clausura sólo con ambos exits cero, manteniendo códigos existentes y verificación estructural. Resta publicar el commit exacto y obtener el check requerido remoto |
