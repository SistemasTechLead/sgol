...
| Bloqueo diagnóstico vigente | El primer run `35403795801` del SHA `304212b5a7b18f82b3fbddaf2662857369448a7a` mantuvo verdes protección de `Fuentes/`, Gitleaks, build, pruebas, formato, vulnerabilidades, TECH-OPS y sonda `1/1`; el gate HU-035 completó referencia, backup y restore, pero el caso positivo cayó en `reconcile-functional-restore` porque el fixture no había generado la key ring portátil que la primera comprobación de seguridad exige. La historia se reconstruyó limpiamente desde `origin/master` en `e4ae7fbae3fd0733e2c7e801b321d106ab6819c4`, PR `#57`; su primer run `35452978805` aprobó todos los pasos previos y el probe restaurado de key ring, pero la reconciliación positiva volvió a terminar con `exit=1`. Se aprobó publicar únicamente caso, exit y código cerrado reconocido para demostrar la causa restante sin exponer salida original. La corrección debe superar el gate integral Linux AMD64 sobre un nuevo SHA publicado |
| Validación local de las correcciones | La corrección aprobó sintaxis y compilación del proyecto con el archivo condicional AMD64 habilitado, cero errores/advertencias; regresión de fixture y key ring portátil con PostgreSQL real `1/1`; red y wrapper `80` aserciones; diagnóstico `216`; stream puro `7/7`; contrato HU-035 `5/5`; sonda real de réplica `1/1`; reproducción/regresión del lector funcional y atribución de `EXPORT_BACKUP_IO_ERROR` con PostgreSQL real `1/1`; sonda Docker de stdin sin/con `--interactive`; build Release `23/23`, cero errores/advertencias; Unit `26/26`; Architecture `5/5`; validadores HU-035 y TECH-OPS; formato, protección local de `Fuentes/` y `git diff --check`. Gitleaks y espejo byte-a-byte permanecen a cargo del runner Linux: Gitleaks no está instalado localmente y `core.autocrlf=true` materializa las copias raíz en CRLF mientras `Fuentes/**` permanece binario/LF; no hay cambios Git dentro de `Fuentes/` |
