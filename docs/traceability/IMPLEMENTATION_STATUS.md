...
| Causas raíz y correcciones | El run `35386054702` demostró que un UUID plano violaba el checkpoint JSON objeto; el run `35388790792` demostró `FINAL_ONLY / OUTSIDE_FINAL_NETWORK / PRESENT / POSITIVE` por la dirección temporal anunciada de SeaweedFS; y el run `35398125435` sobre `e8b7befc8b5896bf2b33d0c2d8c6ea663dc2e08c` demostró `REFERENCE_CAPTURE_SNAPSHOT_POSTGRESQL_ERROR`. La reproducción enfocada real precisó SQLSTATE `42703`: `AssertMigrationAsync` consultaba `migration_id`, inexistente en `__EFMigrationsHistory`, cuya columna estándar es `"MigrationId"`. Las correcciones mínimas usan checkpoint cerrado, alias final estable e identificador EF exacto, sin alterar reintentos, estados, operaciones S3, topología ni alcance productivo |
| Bloqueo diagnóstico vigente | La corrección de `"MigrationId"` debe superar el gate integral Linux AMD64 sobre el SHA publicado. El run causal `35398125435` mantuvo verdes protección de `Fuentes/`, Gitleaks, build, pruebas, formato, vulnerabilidades, TECH-OPS y la sonda `1/1`, pero no acredita cierre porque HU-035 se detuvo durante la preparación del caso positivo |
| Validación local de las correcciones | La corrección final aprobó sintaxis; red `77` aserciones; diagnóstico `216`; stream puro `7/7`; contrato HU-035 `5/5`; sonda real de réplica `1/1`; reproducción/regresión del lector funcional con PostgreSQL real `1/1`; build Release `23/23`, cero errores/advertencias; Unit `26/26`; Architecture `5/5`; validadores HU-035 y TECH-OPS; formato, protección local de `Fuentes/` y `git diff --check`. Gitleaks y espejo byte-a-byte permanecen a cargo del runner Linux: Gitleaks no está instalado localmente y `core.autocrlf=true` materializa las copias raíz en CRLF mientras `Fuentes/**` permanece binario/LF; no hay cambios Git dentro de `Fuentes/` |
