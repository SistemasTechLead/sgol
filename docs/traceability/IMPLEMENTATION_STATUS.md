...
| `FRONT-009` | Fila 77 de Adenda 45; `HU-008`/`CAP-009`, `CA-008`/`CP-008-P/N`, F06 y Adenda 49; extensión UI-C04 y resolución consumidora BR-M13 aprobadas el 2026-09-25 | `origin/master` `63a8f7421296c02bf740cf4c462f219fcd3eec5c`; worktree limpio `codex/front-009` sin commits de PR `#74`; excepción de orden aprobada sólo para integrar antes de FRONT-008 | `Implementada localmente`: UI-C04 en `/configuracion` lista historia, crea borrador y publica con fecha UTC derivada de `America/Mexico_City`, motivo, CSRF, idempotencia e `If-Match`; 412 exige recarga explícita. `GET configuration/releases` entrega `meta.count` al cliente común; la proyección de sesión añade `PER-CONFIG-ADMIN` sólo para Dirección. El servidor conserva autorización, solapamiento, historia y auditoría. Mapa y comprobaciones en `FRONT_009_RELEASES.md`. Sin commit, push, PR ni merge; no se declara `Publicada` o `Integrada` |
