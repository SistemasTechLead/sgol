...
| Diagnóstico causal de PUT y despacho REFERENCE | Adenda 40 y complemento aprobados el 2026-09-18: amplían el artifact existente con categorías cerradas del PUT y clasificaciones sanitizadas de red, dirección anunciada, nodo y capacidad después del fallo y antes de limpieza; el complemento registra caso, resultado/error del outbox y estado/error del job con valores cerrados cuando REFERENCE falla. El run `35383233043` sobre `e3a31ebdc2527ef4a720dd9447828f3c00b5b075` no reprodujo el HTTP 500: mostró red final, dirección final, nodo presente y capacidad positiva, pero falló en REFERENCE con `RetryScheduled`; no cambia S3, outbox, job, reintentos, timeouts ni matriz y no acredita cierre de HU-035 |
| Causa raíz y corrección | El run `35386054702` sobre `e4f64b7abe2174f36d7836aabd7481339d42652a` demostró que el caso positivo devolvía `RETRY_SCHEDULED / RECOVERY_REFERENCE_JOB_FAILED` sin fila de job. El handler enviaba un UUID plano como checkpoint, incompatible con el constraint que exige un objeto JSON; la inserción se revertía antes de persistir el run. La corrección mínima codifica y valida el checkpoint como objeto con el único campo `reconciliationId`; conserva lógica, reintentos, estados, errores y alcance productivo |
| Validación local de la corrección | Compilación enfocada sin errores ni advertencias; regresiones puras del checkpoint y diagnóstico `2/2`; job operativo `1/1`; arquitectura de continuidad `5/5`; validador HU-035 y diagnóstico de servidor `215` aserciones aprobados. El primer build de solución no encontró ocho `project.assets.json`; tras un único restore locked, `dotnet build SGOL.slnx --no-restore --configuration Release` aprobó los 22 proyectos sin errores ni advertencias. Formato y protección local de `Fuentes/` aprobados; el gate Linux AMD64 exacto sigue requerido antes del cierre |
