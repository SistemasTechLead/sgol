...
| `FRONT-007` | Fila 70 de Adenda 45; `HU-007`/`CAP-007`, `CA-007`/`CP-007-P/N`; F06 y Adendas 41, 46 y 48; extensión de diseño UI-I06/I07 aprobada expresamente | `FRONT-006` integrada mediante PR `#72`; contrato GET de rol e historia, reset con generación única y MFA reciente aprobados para este consumidor | `Publicada` mediante PR `#73` desde `codex/front-007`, sin merge. Dirección consulta rol vigente e historia, asigna, cambia o revoca con CSRF, idempotencia y `If-Match`; reset MFA motivado exige MFA reciente en endpoint y servicio, genera contraseña temporal de visualización única y conserva auditoría e invalidación. Build Release, pruebas enfocadas y capturas sanitizadas en `FRONT_007_ROLES_MFA.md`; pipeline del SHA final pendiente de verificación e integración no ejecutada |
| `FRONT-008` | Fila 76 de Adenda 45 | PR draft `#74`, `codex/front-008`, cabeza `6f9a1ef7870e3160e497af14b1702995b6960fcc` verificada el 2026-09-25 | Avance parcial no integrado: consulta de `LOR-001`, semana ISO, calendario y edición de días de una release `BORRADOR` existente. El recorrido de primera release y BR-API01 siguen pendientes en su propia tarea; este registro no declara FRONT-008 terminada |
| `FRONT-009` | Fila 77 de Adenda 45; `HU-008`/`CAP-009`, `CA-008`/`CP-008-P/N`, F06 y Adenda 49; extensión UI-C04 y resolución consumidora BR-M13 aprobadas el 2026-09-25 | Rama `codex/front-009` desde `origin/master` `63a8f7421296c02bf740cf4c462f219fcd3eec5c`, sin commits del PR `#74`; excepción de orden aprobada para integrar antes de FRONT-008 | `Publicada` en PR draft `#75`, sin merge. UI-C04 en `/configuracion` lista historia, crea borrador y publica con fecha UTC derivada de `America/Mexico_City`, motivo, CSRF, idempotencia e `If-Match`; 412 exige recarga explícita. `GET configuration/releases` entrega `meta.count` al cliente común; la proyección de sesión añade `PER-CONFIG-ADMIN` sólo para Dirección. El servidor conserva autorización, solapamiento, historia y auditoría. Mapa y comprobaciones locales en `FRONT_009_RELEASES.md`. Pipeline del SHA final pendiente de verificación; no se declara `Integrada` |
