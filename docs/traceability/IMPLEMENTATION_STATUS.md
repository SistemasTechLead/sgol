...
| Punto de avance | `FRONT-002` fue ordenada expresamente y se implementó localmente en `codex/front-002` sin commit ni publicación; `TECH-FRONT-005` y las historias posteriores no se inician |
...
| `FRONT-002` | `F07_ADENDA_45_BACKLOG_FRONTEND_DEFINITIVO.md`, Adendas 41 y 46 | `FRONT-001` integrada; BR-N01..N08 aprobadas; BR-D03/D15 aprobadas e incorporadas en `docs/design`; BR-D16 consumida | `Implementada localmente` en `codex/front-002`, sin commit ni publicación: UI-A06/A07, encabezado de sesión, `/` y `/mi-trabajo` protegido como anfitrión vacío, logout POST/CSRF, avisos seguros y limpieza de sesión. Ninguna UI-E01..E08 implementada ni visible. Restore bloqueado; build Release `26/26` sin errores/advertencias; unitarias enfocadas `47/47`, arquitectura `9/9`, smoke de servidor Kestrel HTTPS/PostgreSQL `1/1` y `FRONT_BROWSER` completo `4/4` en escritorio/móvil con cuatro roles y cleanup. Formato dirigido y `git diff --check` correctos. Comandos, matriz y límites en `docs/traceability/FRONT_002_SESION_Y_SHELL.md`. Suite integral y pipeline remoto no ejecutados |
| `FRONT-003..020` | `F07_ADENDA_45_BACKLOG_FRONTEND_DEFINITIVO.md` | Según orden y dependencias de la Adenda 45 | `No iniciadas`; cada brecha bloquea sólo su historia consumidora |
