...
| Bloqueo diagnóstico vigente | El run `35454375124` del SHA `25f92a03447bca76e2fca82083c91b7bcb2d4aee` aprobó protección de `Fuentes/`, Gitleaks, build, suites, formato, vulnerabilidades, imagen, TECH-OPS y sonda de réplica, y demostró `positive / exit=1 / RECONCILIATION_IMMUTABLE_CONFLICT`. El replay idéntico era bloqueado antes de leer el reporte: JSON conserva ticks de 100 ns, PostgreSQL persiste `timestamptz` a microsegundos y `MarkRestoreStartedAsync` comparaba el instante original sin normalizar contra el persistido. La corrección normaliza esa frontera a UTC/microsegundos, conserva el hash y mantiene el rechazo de instantes o hashes distintos. Debe superar el gate integral Linux AMD64 sobre un nuevo SHA publicado |
| Validación local de las correcciones | La corrección aprobó sintaxis y compilación del proyecto con el archivo condicional AMD64 habilitado, cero errores/advertencias; regresión de fixture y key ring portátil con PostgreSQL real `1/1`; regresión de precisión PostgreSQL y replay inmutable `1/1`; red y wrapper `84` aserciones; diagnóstico `216`; stream puro `7/7`; contrato/diagnóstico/red pura HU-035 en OperationsIntegration `54/54`; sonda real de réplica `1/1`; reproducción/regresión del lector funcional y atribución de `EXPORT_BACKUP_IO_ERROR` con PostgreSQL real `1/1`; sonda Docker de stdin sin/con `--interactive`; build Release `23/23`, cero errores/advertencias; Unit `26/26`; Architecture `5/5`; validador HU-035; formato, protección local de `Fuentes/` y `git diff --check`. Gitleaks y espejo byte-a-byte permanecen a cargo del runner Linux: Gitleaks no está instalado localmente y `core.autocrlf=true` materializa las copias raíz en CRLF mientras `Fuentes/**` permanece binario/LF; no hay cambios Git dentro de `Fuentes/` |
