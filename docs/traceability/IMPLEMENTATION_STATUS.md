...
| Punto de avance | `FRONT-002` integrada mediante PR `#68`; `FRONT-003` integrada mediante PR `#69`, cabeza `65f3b6bc2a4652f7eb92d167e88b9d079444d702`, run `36041719411` con `TECH-BASE-003 / PR gates` `SUCCESS` para ese SHA y merge `9dba2ed71841c7df443ea773bd986208928d98d1`; cabeza y merge verificados como ancestros de `origin/master` el 2026-09-24. `FRONT-004` implementada localmente en `codex/front-004`; `FRONT-005/006`, `TECH-FRONT-005` y tareas posteriores no iniciadas |
...
| `FRONT-004` | Fila 67 de Adenda 45; `HU-001/HU-002`, `CA/CP-001/002`, Adenda 46; extensión mínima de `docs/design/estados-y-mensajes.md` aprobada expresamente para UI-I02 | `FRONT-003` integrada; detalle y cliente común existentes; BR-D04 aplicable | `Implementada localmente` en `codex/front-004`: UI-I02 edita empleo/puesto/turno, baja y reactiva mediante API real con CSRF, `Idempotency-Key` por intención y `If-Match`. Conserva historia; 412 exige recarga explícita. Build Release, unitarias de personas `8/8`, arquitectura de interfaz `9/9`, PostgreSQL de personas `14/14` y `FRONT_BROWSER` completo `6/6` locales. Mapa y límites en `FRONT_004_EMPLEO_Y_VIGENCIA.md`. Sin commit, push, PR ni merge |
| `FRONT-005..020` | `F07_ADENDA_45_BACKLOG_FRONTEND_DEFINITIVO.md` | Según orden y dependencias de la Adenda 45 | `No iniciadas`; cada brecha bloquea sólo su historia consumidora |
